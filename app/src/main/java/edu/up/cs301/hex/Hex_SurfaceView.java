...
public class Hex_SurfaceView extends SurfaceView implements View.OnTouchListener {
    ...
    protected void onDraw(Canvas canvas) {
        ...
        //draws the blue side
        float surfaceViewHeight = getWidth();
        drawTriangle(surfaceViewHeight, hexRedSide, hexBlueSide, canvas); //left
    ...
    public HexTile getNearestHex(float xPos, float yPos)   {
        ...
        // Returns the HexTile with shortest hyp
        return closestSoFar;
    }
    ...
    // Note to not draw if the smallest hyp is less than the radius

    /**
     * Draws the entire 11x11 Hex grid. Uses the lineTo method to draw 6 points for an individual hexagon
     */
     ...
     */
    private void drawTriangle(float height, Paint red, Paint blue, Canvas canvas) {
...
//        canvas.drawPath(path, paint);
        ...
        p1Path.moveTo(0, height_SurfaceView / 3f);
        p1Path.lineTo(width_SurfaceView, height_SurfaceView / 3f);
        p1Path.lineTo(width_SurfaceView, 2 * height_SurfaceView / 3f);
        p1Path.lineTo(0, 2 * height_SurfaceView / 3f);
        p1Path.close();

        canvas.drawPath(p1Path, red);

        float centerTop = (((width_SurfaceView / 2) + height_SurfaceView) / 3f);
        float centerBottom = width_SurfaceView - centerTop;
        Path p2Path = new Path();

        p2Path.moveTo(centerTop - height_SurfaceView / 6f, 0); // Top left
        p2Path.lineTo(centerTop + height_SurfaceView / 2f, 0); // Top right
        p2Path.lineTo(centerBottom + height_SurfaceView / 6f, height_SurfaceView); // Bottom right
        p2Path.lineTo(centerBottom - height_SurfaceView / 2f, height_SurfaceView); // Bottom left

        canvas.drawPath(p2Path, blue);
